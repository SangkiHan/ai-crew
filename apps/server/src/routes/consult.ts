...
export function registerConsultRoutes(app: FastifyInstance) {
  // 기획자의 ask_employee MCP 툴이 호출하는 경로. create_ticket과 같은 이유로 teamId를 받아
  // "다른 팀 직원에게는 못 물어본다"를 서버에서 강제한다.
  app.post<{ Body: ConsultBody }>("/api/consult", async (req, reply) => {
    const { teamId, employeeName, project, question } = req.body;
    ...
    const result = await requestConsultEmployee(employeeName, project, question);
