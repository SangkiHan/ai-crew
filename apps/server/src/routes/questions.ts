...
export function registerQuestionRoutes(app: FastifyInstance) {
  app.post<{ Body: { text: string } }>("/api/questions", async (req, reply) => {
    const { text } = req.body;
    if (!text) return reply.code(400).send({ error: "text is required" });
    return createQuestion(text);
  });

  app.get<{ Querystring: { status?: string } }>("/api/questions", async (req) => {
    return listQuestions(req.query.status);
  });

  app.get<{ Params: { id: string } }>("/api/questions/:id", async (req, reply) => {
