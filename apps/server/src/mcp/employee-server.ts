...
server.tool(
  ...
  "다른 프로젝트를 담당하는 직원에게 그 프로젝트의 실제 코드를 기반으로 질문하고 답을 받습니다 " +
    ...
    "쓰세요. 티켓에 나온 대상을 특정할 수 없을 때도 이 툴을 쓰세요 - 예를 들어 '등록화면에 비고 " +
    "추가'처럼 화면 기준으로 내려온 작업인데 그 화면을 볼 수 없다면, 화면 담당 직원에게 어떤 " +
    "화면인지·지금 호출하는 API가 무엇인지·필드 이름과 타입이 무엇인지를 물어 합의한 뒤에 " +
    "시작하세요(짐작으로 만들면 화면과 안 맞습니다). 그 직원은 코드를 수정하지 않고 조사만 해서 " +
    "답합니다. 아직 구현 전이면 '아직 없다'고 정직하게 답합니다 - 미래에 어떻게 만들어질지까지 " +
    "알려주지는 않으니, 사전에 팀장이 스펙을 못박아주지 않은 새 기능이라면 이 답만으로 통합을 " +
    "확정하지 말고 담당 직원과 계속 소통하세요.",
  {
    ...
    question: z
      .string()
      .describe(
        "구체적인 질문 (예: 'remark 필드가 실제 응답에 어떤 이름/타입으로 내려오나요?', " +
          "'등록화면이 어느 파일이고 저장할 때 어떤 API를 호출하나요? 비고 항목을 추가하려면 " +
          "요청 바디에 어떤 필드명으로 넣으면 되나요?')"
      ),
  },
