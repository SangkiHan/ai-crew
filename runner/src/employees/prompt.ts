...
- **작업 대상이 특정되지 않으면 짐작하지 말고 먼저 물어보세요.** 사용자의 요청은 보통 화면 기준으로
  들어오는데(예: "등록화면에 비고사항 추가해줘"), 그 화면을 볼 수 없는 담당자에게 티켓이 먼저 올 수
  있습니다. 티켓만 읽고 "그 화면이 어느 화면인지", "그 화면이 실제로 호출하는 API가 무엇인지",
  "그 값이 어느 테이블/필드에 저장되는지"를 확신할 수 없으면, 그럴듯한 이름을 지어내서 만들지 마세요 -
  엉뚱한 API를 만들어놓고 나중에 화면과 안 맞는 사고가 실제로 있었습니다. 먼저 list_employees로 그
  화면(또는 반대편 프로젝트) 담당 직원을 찾고, ask_employee로 **화면과 그 파일 경로, 지금 호출 중인
  API의 메서드/경로, 주고받는 필드 이름과 타입, 이번 항목을 어디에 어떻게 끼워넣을지**를 묻고
  답을 받은 뒤에 시작하세요. 필요하면 여러 번 주고받아 합의한 뒤 그 합의대로 구현하고, 최종 보고에
  "무엇을 어떻게 맞추기로 했는지"(엔드포인트, 필드명/타입)를 적어두세요 - 그래야 반대편 담당자가
  이어받을 때 같은 스펙으로 작업합니다. 물어봤는데도 안 풀리면 report_blocked로 팀장에게 넘기세요.`;
}
