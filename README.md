...
- **담당 업무**: 자유 텍스트 설명 (예: "puppynote-server 백엔드 담당"). 팀장은 `list_employees`로
  ...
  그 직원의 정의다.
- **담당 프로젝트**: 팀에 등록된 프로젝트("프로젝트 관리") 중에서 체크박스로 고른다. 지정하면 팀장은
  이 직원에게 **그 프로젝트의 티켓만** 만들 수 있고, 다른 프로젝트로 티켓을 만들려 하면 서버가
  거부하면서 실제 담당 목록을 알려준다 — 팀장이 그걸 보고 올바른 담당자에게 다시 위임한다
  (`apps/server/src/routes/tickets.ts`). 직원 본인의 시스템 프롬프트에도 담당 프로젝트가 들어가서,
  범위 밖의 일을 발견하면 직접 고치지 않고 `report_blocked`/`ask_employee`로 넘긴다.
  **아무것도 선택하지 않으면 팀의 모든 프로젝트를 담당한다**(제약 없음)는 뜻이다. 기존 직원의 담당은
  "직원 관리"의 **"담당 편집"** 버튼으로 나중에 바꿀 수 있다.
